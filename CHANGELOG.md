...
### Added
- **Copy button & copying menu item.** `ButtonCopy` and `MenuItemCopy` copy to the clipboard on activation: pass `copy` for a literal string, or `copyNode` (bare → nearest `data-copy-source` ancestor; a selector → `closest()`) to copy a DOM region as rich text (`text/html`) + plain text. The write lives in the web component (`<a-button>` / `<a-menu-item>`), which announces the result on a bubbling `copydone` event; the wrapper flips the leading icon to a check / ✕ and retones to `success` / `critical` for ~2s. `copyLazy` keeps the content out of the DOM until the click — the element fires `copyrequest`, you set `copy`, and the write completes inside the click's activation window. `onCopied(ok)` / `onCopyRequest` callbacks. The same `copy` / `copy-node` / `copy-lazy` attributes work on plain `<Button copy=…>` / `<MenuItem copy=…>` and hand-authored `<a-button>` / `<a-menu-item>` (the icon/tone feedback is the preset wrappers' part). Exports `ButtonCopy`, `MenuItemCopy`, `ButtonCopyProps`, `MenuItemCopyProps`, and the `CopyMode` prop type.
