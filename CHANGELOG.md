...
### Changed
- **A `quaternary` `<a-button>` holds its rest foreground when pressed.** The quaternary `:active` state dropped the light-mode `+0.05` lightness bump (dark already mirrored rest), so pressing a quaternary button no longer shifts its label colour — active now matches rest in both themes. Hover and `selected` still deepen to the quaternary hover foreground. Only the lowest-emphasis priority changes; primary/secondary/tertiary active states are unchanged.
