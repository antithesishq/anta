...
### Changed
- **A `quaternary` `<a-button>` holds its rest foreground when pressed.** The quaternary `:active` state now pins the foreground to the rest value, so pressing a quaternary button no longer shifts its label colour — active matches rest in both themes. This is an explicit `:active` rule ordered after the `:hover` block, so it wins the source-order tie while a pressed button is also `:hover` — previously the absence of an `:active` rule let the hover foreground bleed through the press on hover-capable devices. Hover and `selected` still deepen to the quaternary hover foreground. Only the lowest-emphasis priority changes; primary/secondary/tertiary active states are unchanged.
