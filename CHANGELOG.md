...
- **`<a-button>` activates on key release (keyup), not keydown.** Enter and Space now fire the button on `keyup` (`keydown` prevents the Space page-scroll and arms the release). This matches the native `<button>` (Space already activated on release), makes keyboard activation **cancelable** (move focus before releasing → no fire), and is what gives lazy copy a keydown→keyup gap to resolve fresh content. Activation is imperceptibly later and no longer key-repeats (it already didn't); programmatic `.click()` is unaffected.
