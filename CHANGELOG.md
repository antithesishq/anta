...
### Added
- **`InputAutocomplete`** — a text field whose value is a **free string**, with a suggestion dropdown that assists without constraining: typing something not in the list is valid and stays the value; picking a suggestion just fills the field. (For a value that must be one of the options, use `Select` with `filter`.) `suggestions` are bare strings or `SelectOption`s (`value` / `label` / `hint` / `icon` / `tone` / `disabled`); `filter` is `true` (built-in case-insensitive substring), a custom `(option, query) => boolean`, or `false` to show suggestions verbatim (async / remote). Fires `onValueChange(value)` as the text changes and `onSelect(option)` when a suggestion is chosen; controlled (`value`) or uncontrolled (`defaultValue`). Free-text friendly keyboard: no suggestion is auto-highlighted, so Enter commits the typed text; ArrowDown highlights the first suggestion, then Enter picks it. Supports `size` / `status` / `statusIcon` / `tone` / `clearable` / `name` / `label` / `hint` / `leading` / `trailing`. Composed from `Input` / `Menu` — no new element. Exposes a proper ARIA combobox (`role="combobox"` field controlling a `listbox` of `option`s); Tab closes the list and moves to the next field, Home / End move the text caret (PageUp / PageDown jump the list), and picking a suggestion with the mouse keeps focus in the field. Exports `InputAutocompleteProps`.
- **`Menu` / `MenuItem` / `Input` gain an optional `role` prop.** Overrides the element's default ARIA role (e.g. `Menu`'s `menu` → `listbox`, `MenuItem`'s `menuitem` → `option`) so a composed control can reparent them under a different role. `InputAutocomplete` uses it to present its dropdown as a combobox `listbox`.

### Fixed
- **`Tooltip` closes when its content is emptied while shown.** The empty gate (no element children, no non-whitespace text) ran only at show time, so a bubble whose content was cleared out from under it — a reactive re-render dropping the text or children under one persistent anchor — stayed up as a blank frame. It now watches the slotted content while open and self-hides once empty, the mirror of the existing "populated later, opens normally" behaviour. Catches in-place text updates too (a renderer setting a text node's value to `""` rather than removing the node).
...
- **`SelectFaceted` gains a `toneSelected` prop.** Tones a selected option row in the facet flyouts (label, selected tint, and the check / checkbox indicator), matching `Select`'s `toneSelected` — e.g. `toneSelected="brand"`. Defaults to a neutral selection.
- **`SelectFaceted` `multiple` facets gain the Alt/⌥-click "isolate" accelerator + hint.** A `multiple` facet's option rows (in the facet flyout and in the flattened global-search results) now support Alt/⌥-click to select only that option (clear the rest), with a default "select only this" tooltip teaching it — matching `Select`. Coupled to the facet's `selectAll` (on by default), so `selectAll: false` drops both. Override a row's tooltip via its `tooltip` (or `''` to suppress).
- **`Select` `selectAll` now defaults on for `multiple` selects.** A `selection="multiple"` Select shows the "Select all" top row (and the Alt/⌥-click isolate accelerator + hint that ride on it) without opting in; pass `selectAll={false}` to drop them. Was opt-in (`selectAll` defaulting to off).
...
### Added
- **`SelectFaceted`** — a faceted filter control: one trigger opens a menu of **facets** (dimensions), each a submenu flyout with its own editor. Facet kinds: `single` (pick one), `multiple` (pick many, with an opt-in `selectAll` row), `text` (free-form substring, applied on Enter / blur), and `custom` (bring your own editor and value, including object-shaped values like a date range or numeric comparator). The value is a `Record<facetKey, value>`, so the same option value under two facets — assignee "alice" and owner "alice" — stays distinct (identity is `(facetKey, value)`). Controlled (`value` + `onValueChange`, whose second argument is an attrs snapshot of what changed) or uncontrolled (`defaultValue`). `searchable` adds one global search field that flattens every options facet into a grouped result list; `clearable` (default on) adds per-facet "Clear" plus a "Clear all" row; `renderTrigger` swaps the default active-facet-count Button. Composed from `Menu` / `Input` / `Tag` and `Select`'s option model — no new element. Exports `SelectFacetedProps` / `SelectFacet` (with the per-kind `SelectFacetSingle` / `SelectFacetMultiple` / `SelectFacetText` / `SelectFacetCustom` / `SelectFacetCustomContext`) / `SelectFacetedValue` / `SelectFacetedChangeAttrs` / `SelectFacetedTriggerState`.
...
- **`Tabs` sizing + track restyle.** A tab is now a prescribed height (`min-height` = 24/28/32, matching Button's scale) rather than one derived from padding, and the strip carries no padding by default — so a strip is exactly a same-size button / input tall at every priority (primary, secondary, tertiary), and `InputDate`'s time toggle matches its inputs by construction. Rings are crisp 0.5px inset outlines: the track edge uses `--tabs-track-border` (`--border-4`) and the selected tab (primary pill + secondary fill) uses its own `--tab-selected-border` (`--border-2`), each an independent, tone-aware knob. For a roomier "well" look, override the strip's `padding` (and bump `border-radius` to match) in plain CSS.
