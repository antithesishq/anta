...
### Added
- **`InputTime` (`<a-input-time>`)** — a segmented wall-clock time field: one boxed input (matching the other inputs' chrome) with separate hour / minute / (12-hour) AM-PM sections, each a `role="spinbutton"`. <kbd>↑</kbd>/<kbd>↓</kbd> steps with wrap, <kbd>←</kbd>/<kbd>→</kbd> moves between sections, typing digits auto-advances, and AM/PM toggles with arrows or `a`/`p`. The clock (12h/24h), section order, separator, and AM/PM text are locale-derived via `Intl` (overridable with `hour12`); 24-hour locales drop the AM/PM section. Value is a 24-hour `"HH:mm"` string, controlled (`value` + `onValueChange`) or uncontrolled (`defaultValue`), form-associated under `name`. `min` / `max` (24-hour `"HH:mm"`) clamp a complete value to the range on step / blur and flag `rangeUnderflow` / `rangeOverflow`. Supports `size` / `status` / `tone` / `disabled` / `clearable`, and exposes `field` / `segments` / `label` / `hint` / `clear` / `trailing` parts. Not SSR-safe (reads `navigator.language`).
