...
### Added
- **`Card` (`<a-card>`)** — a surface container: a bordered, toned box that lays out an optional full-bleed `media` region (`mediaPosition` `top` / `bottom` / `left` / `right`) plus a `header` / body / `footer` stack, with a top-right `actions` slot and a **left-aligned** footer. `tone` (named or any CSS colour) and `priority` (`secondary` / `primary` / `tertiary`) drive the surface; `size` scales padding; `selected` draws an inset ring; `loading` shows a skeleton pulse. Pass `href` to turn the whole card into a link — the shadow container becomes a focusable anchor whose accessible name comes from the header → body → URL. Header / body / footer are respected as passed; the element supplies default typography. `::part(container | media | content | header | actions | body | footer)`. Exports `CardProps`.
