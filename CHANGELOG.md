...
### Added
- **`Dialog` (`<a-dialog>`)** — a modal dialog / edge drawer built on a native `<dialog>` in shadow DOM, so the top layer, focus trap, focus return, `::backdrop`, and Esc all come from the platform. Three optional zones — `header`, body (`children`), and `footer` (a right-aligned action row) — plus a top-right ✕ close button (`closable`, on by default). `position` places it: `center` (default modal), `left` / `right` / `top` / `bottom` edge drawers that slide in (full height for left/right, full width for top/bottom), or `fullscreen` (edge-to-edge, no gap or radius). `--dialog-margin` (32px) is the minimum overlay kept from the far viewport edge so the backdrop stays visible — all four edges for the centered modal, the one exposed edge for a drawer (capping its slide-axis size). Open state follows Anta's shared contract: controlled (`open` + `onStateChange`) or uncontrolled (`defaultOpen`, or the `name` + `data-dialog-open` / `data-dialog-close` trigger convenience), with a cancelable `statechange` fired before every user open / dismiss. `persistent` disables light dismiss (Esc + backdrop) for alert / confirm dialogs. `round` sets the corner radius: a side keyword (`top` / `right` / `bottom` / `left`) rounds that edge's corners (pairs with a drawer's exposed edge), a number / string (including a full `border-radius` shorthand) rounds all corners, bare `round` uses `--dialog-radius`; default is rounded center, square drawers. Surface is `--bg-1` above a themed `--dialog-overlay`, lifted by a composed `box-shadow` whose first inset layer is the hairline border. Enter / exit transitions (scale-fade for the modal, slide for drawers) are gated under `prefers-reduced-motion`. On open, focus goes to the dialog itself (Tab to reach the first control; `autofocus` on a control opts into focusing it), and the dialog takes its accessible name from the `header`. Styleable via shadow parts — `::part(dialog)` (surface), `::part(dialog)::backdrop` (overlay), and `::part(header|body|footer|close)` — plus plain CSS; a `::part` rule overrides the internal styles without `!important` (scope it to a `[position]` / class so drawers keep their full-bleed cross axis).
