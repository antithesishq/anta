...
### Fixed
- **`Tooltip` renders its trigger before the element upgrades.** `a-tooltip:not(:defined)` was `display: none`, which hid the wrapped trigger (the tooltip's child) until the JS registered — invisible on SSR / first paint, then popping in. It's now `display: contents` (matching the shadow `:host` once defined), so the trigger shows immediately; the bubble is still built from the `content` attribute on upgrade, so nothing leaks pre-upgrade.
