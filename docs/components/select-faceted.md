...
`SelectFaceted` uses [Menu](./menu.md), [Select](./select.md)'s option model,
[Input](./input.md), and [Tag](./tag.md). It does not create a custom element.
...
Open **Recency** in the first demo, select **Custom range**, then pick **From**
and **To** from the fields side by side. Each field closes its own calendar after
a pick while the Recency editor stays open for the second date. Both fields have
a clear button. Open **Min duration** to choose a comparison, then enter seconds.
The comparison alone does not apply the facet or add a result chip.

Open **Metadata** to choose key–value tags such as
...
```tsx
const recencySummary = (v) =>
  'preset' in v
    ? { today: 'Today', yesterday: 'Yesterday', last14: 'Last 14 days', last30: 'Last 30 days' }[v.preset]
    : v.from && v.to ? `${v.from} → ${v.to}` : 'Custom range'
const sign = (comparison) => ({ gt: '>', gte: '≥', eq: '=', lte: '≤', lt: '<' })[comparison] ?? '≥'
...
const FACETS = [
  ...
  { key: 'duration', label: 'Min duration', kind: 'custom', summary: (v) => `${sign(v.comparison)} ${v.min}s`, render: /* DurationEditor with draft comparison and Input value */ },
  { key: 'recency',  label: 'Recency', kind: 'custom', summary: recencySummary, render: /* radio presets and two InputDate fields */ },
]
      ...
      {active.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          ...
          {active.map((facet) => {
            ...
            // Free-form text / duration → an editable Input.
...
Option facets use an editable [Select](./select.md) with the facet label in its
`leading` prefix. Recency uses a summary tag and is edited in the facet menu.
...
This editor uses `MenuItem` rows with radio indicators for the presets.
Selecting **Custom range** reveals two `InputDate` fields side by side. The facet
value is either `{ preset }` or `{ from, to }`. The `resolveRange` helper
converts either shape into the date range used to filter the result set. Each
`InputDate` calendar opens above the facet menu without closing it.
  ...
  render: ({ value, onChange }) => {
    ...
    return (
      <div style={{ minWidth: 360 }}>
        ...
        {mode === 'custom' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, padding: 8 }}>
...
properties as [Select](./select.md). They use `kind` in place of Select's
