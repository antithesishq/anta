...
one-second hover, a tooltip beside the row describes the shortcut. Moving to
another row restarts the delay. An option's `tooltip` replaces that hint; set it
to `''` to hide the hint.
