...
Explore the series below, then select a preview to see its options and examples.
...
Hover the translucent teal bars to see their color intensify. The purple bar sets `highlight: false`, so its appearance stays unchanged. The gray bar uses `hoverable: false`, so it does not participate in hit testing.
...
Hover a teal rectangle for the default tooltip, or a purple rectangle for custom content showing its bounds. Where the two series overlap, both tooltip entries appear together, separated by a divider. The tooltip follows the pointer.
...
Select a point to display its source data beneath the plot. `on_select` receives the selected point and its original row, so you can update a detail panel or application state.
...
`viewport` requests a window at mount and whenever its `key` changes. Increment the key to apply a request again, even after the user has moved elsewhere. Set an axis to `null` to restore its full extent; omit it to leave its window unchanged.

`on_viewport_change` reports the visible and full ranges. Keep this notification separate from viewport requests to avoid reapplying the view on every gesture. See [Viewport](#viewport) and [Viewport changes](#viewport-changes).
