...
- `src/components/` contains the Anta JSX `Plot` and `PlotSurface` wrappers, exposed through `/components`. They must not register elements or access the DOM. Use Anta’s configured hooks for tooltip state. Browser verification covers React 19 and Preact, including cleanup and discarded renders.
