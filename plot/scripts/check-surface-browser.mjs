...
export async function checkSurface() {
    ...
    const highlight = surface.highlight
    ...
    check(
        transferred.canvas.width === 1280 && transferred.highlight.width === 1280,
        'presentation preserves worker backing stores',
    )
    ...
    surface.configureCapture({
        wheel_capture: 'both',
        wheel_modifier: 'ctrl',
        wheel_activation: 'hover',
        wheel_delay: 0,
        wheel_tolerance: 0,
        wheel_reset_on_move: false,
        pointer_capture: 'mouse',
        pointer_buttons: [0],
        pointer_threshold: 3,
        pointer_modifier: 'ctrl',
    })
    ...
    try {
        ...
        check(
            result.width === 900 && result.highlight === 900 && result.color === 'display-p3',
            'both canvases transferred to a real worker',
        )
    } finally {
        worker.terminate()
    ...
    const data = [{ x: 5, y: 5 }]
    plot.plotArgs = {
        axis: { x: { min: 0, max: 10 }, y: { min: 0, max: 10 } },
        zoom_pan: true,
