...
a clear button. Open **Min duration** to choose a comparison from its dropdown.
...
const FACETS = [
  ...
  { key: 'duration', label: 'Min duration', kind: 'custom', summary: (v) => `${sign(v.comparison)} ${v.min}s`, render: /* Select comparison and Input value */ },
