---
title: Slider
description: Select one number or an interval with relative rail dragging.
nav:
  icon: settings-2
  group: controls
  order: 60
...
import SliderEditableValueDemo from '../../../components/SliderEditableValueDemo'
import SliderRangeDemo from '../../../components/SliderRangeDemo.astro'
...
```tsx folded title="Controlled range"
...
<Preview direction="column" align="stretch" background="var(--bg-2)">
  <SliderEditableValueDemo client:load />
  <style is:inline>{`
    .editable-slider {
      display: grid;
...
```tsx folded title="Slider with an editable value"
import { useState } from 'preact/hooks'
...
```css folded title="Editable Slider layout"
.editable-slider {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 72px;
  align-items: center;
