...
circle. The indicator's size and ring are `--avatar-status-size`,
`--avatar-status-ring-width`, and `--avatar-status-ring`; its color comes from
`status`, so route color through the prop rather than overriding
`--avatar-status-color`.

<Preview gap="12px">
  <Avatar className="round" seed="user-42" name="Vlad Korobov" size={56} status="success" />
  <Avatar className="round" seed="lena-r" name="Lena Rossi" size={56} status="critical" />
  <style is:inline>{`
