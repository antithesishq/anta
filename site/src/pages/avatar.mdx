...
| `headRadiusTop`, `headRadiusBottom` | `0` square to `1` fully round, per half of the head |
...
head with both radii at `1` is a circle. Splitting the head into a top and a
bottom radius is what gives it an egg shape — a round top over a squarer bottom
reads as a head, and the reverse reads as a jaw. Under `any` the head radii
average near `1`, so heads read round unless you narrow them yourself.
