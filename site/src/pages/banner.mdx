...
strip and an `aria-label`. The `<Banner>` wrapper also adds `role="status"` (a polite
live region) for you; hand-authoring, set it yourself so the notice reaches assistive
tech (`role="alert"` for an urgent one).
