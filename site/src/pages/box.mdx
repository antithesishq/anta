...
<Preview align="stretch" gap="16px" justify="flex-start">
  <Box round={8} className="edge" style={{ width: '150px' }}>Content that fits.</Box>
  <Box round={8} className="edge" style={{ width: '150px', overflow: 'hidden', whiteSpace: 'nowrap' }}>
    A label too long for this box.
  </Box>
  <Box round={8} className="edge" style={{ width: '150px', height: '56px', overflowY: 'auto' }}>
...
```tsx folded
<Box round={8} className="edge" style={{ width: 150 }}>Content that fits.</Box>

<Box round={8} className="edge" style={{ width: 150, overflow: 'hidden', whiteSpace: 'nowrap' }}>
  A label too long for this box.
</Box>

<Box round={8} className="edge" style={{ width: 150, height: 56, overflowY: 'auto' }}>
  One. Two. Three. Four. Five. Six. Seven. Eight. Nine. Ten.
</Box>
```
...
siblings, or style them yourself instead of using `fade`.
...
`measurechange` fires once at connection, then after the box or its content
changes. `changed` holds only the fields that moved; `current` is the complete
snapshot.
...
serves every Box, so media queries and the user-agent read run once.
...
`pointer`, `hover`, and `prefers-reduced-motion` media queries for the rest.

Every value below is live. Switch the site theme, resize the window, or open the
page on a touch device. `osVersion` and `browserVersion` are major
