...
the button fires `onCopyRequest(provide)`, you compute the value and call
`provide(text)`, and the element writes it. Useful when the payload is large or
expensive, or arrives from another thread.
...
<ButtonCopy
  copyLazy
  label="Copy report"
  onCopyRequest={(provide) => provide(generateReport())} // computed on click
/>
```
...
Nothing large sits in the DOM — there's no bound `copy` value. On click the
element hands you a `provide` callback; call it with the content, synchronously or
after an `await` (fetching, a worker round-trip). The clipboard API's user-gesture
requirement is preserved because the click's activation window is still live.
Every click is a fresh request, with no state to reset.
