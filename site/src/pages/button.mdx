...
<Preview>
  ...
  <Button round tone="brand" icon="check" label="Confirm" />
...
`IconShape` union — see the [Icon](/icon) page for the full
...
`ButtonCopy` is a copy button: the `<a-button>` element writes to the clipboard
itself on click, and the wrapper flips the leading icon to a check and retones to
`success` for ~2s (✕ / `critical` on failure). Set `copy` for a literal string,
or drop the label for an icon-only copy button.
...
The copy logic lives in the web component, so the button works the same whether
you render `<ButtonCopy>`, plain `<Button copy=…>`, or author `<a-button copy=…>`
by hand — only the icon / tone feedback needs the wrapper. `onCopied(ok)` fires
after each attempt. Override the resting glyph with `icon`; the check / ✕ swap in
only during the feedback window.
...
With `copyLazy`, the content isn't computed or held in the DOM until the click:
the button fires `onCopyRequest`, you compute the value and set `copy`, and the
write completes inside the click's activation window. Useful when the payload is
large or expensive, or arrives from another thread.

<Columns>
<Col>

<Preview minHeight={90}>
...
<ButtonCopy
  ...
  copyLazy
  label="Copy report"
  onCopyRequest={() => setPayload(generateReport())} // computed on click
  onCopied={() => setPayload('')}                     // reset for next click
/>
```
...
The `copy` value starts empty, so nothing large sits in the DOM. On click the
element asks for it (`onCopyRequest`), you set it, and the pending write flushes
once the value lands — the clipboard API's user-gesture requirement is preserved
because the activation is still live. Resetting `copy` on `onCopied` keeps every
click a fresh request.
...
### Wearing the look on your own markup
...
opt-in marker — it stops anta restyling a `role="button"` element it doesn't own
...
`type="submit"`, so inside a `<form>` it submits without any extra wiring:
