...
clipboard when the button is activated, and flips the copy glyph to a check while
retoning to `success` for ~2s (✕ / `critical` on failure). Set `copy` for a
literal string, or drop the `label` for an icon-only copy button.
...
`trailing`. Pass `none` to drop the glyph — the feedback is then a ghost of the
label floating up plus the success / failure tone flash.
...
<ButtonCopy copy={value} label="No icon" iconPlacement="none" />         // ghost + tone flash
```
