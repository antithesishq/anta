...
There's no text-tone prop. To tint the **label and hint**, set `color` on the
checkbox with a theme-aware `--text-N-{tone}` token (and `--checkbox-hint-color` for
the hint). Setting `color` alone tints the label; pair it with `tone` /
`toneSelected` to tint the whole control.

<Preview gap="16px">
  <Checkbox className="toned-text" defaultChecked tone="critical" hint="Deletes everything">Critical</Checkbox>
  <style is:inline>{`
    a-checkbox.toned-text { color: var(--text-1-critical); --checkbox-hint-color: var(--text-4-critical); }
  `}</style>
</Preview>

```css folded
/* Mark via the prop, text via the role tokens. The class is just for the demo —
   swap your own selector. */
a-checkbox.toned-text {
  color: var(--text-1-critical);
  --checkbox-hint-color: var(--text-4-critical);
}
```
...
To tint the label + hint, set `color` (and `--checkbox-hint-color`) with the
`--text-N-{tone}` tokens — there's no text-tone prop. The focus ring is the global
