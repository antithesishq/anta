...
`time` turns it into a date-time field. The value becomes `YYYY-MM-DDTHH:mm`, the
field takes a 24-hour time after a space (`06/15/2026 14:30`, or `2:30pm`), and the
menu grows a time row under the calendar. Picking a day keeps the menu open so you
can set the time; the Done button closes it.

<Preview>
  <InputDateDemo client:only="preact" label="Starts at" time defaultValue="2026-06-15T09:00" style={{ width: '260px' }} />
</Preview>

```tsx folded
<InputDate label="Starts at" time defaultValue="2026-06-15T09:00" />
```
