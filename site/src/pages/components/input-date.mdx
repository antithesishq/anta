...
**`InputDate`** is a date field that replaces the native `type="date"` input: a
text field you can type into, backed by a `Calendar` that opens from the trailing
button. It's the headline of the InputDate family; **`Calendar`**, the month grid it
builds on, is documented lower down. Time entry lands as it ships.
...
time row under the calendar — hours, minutes, and, in 12-hour locales, an AM/PM
toggle, and a confirm (✓) button; picking a day keeps the menu open so you can set the
time. The clock follows the locale (`en-US` is 12-hour, most others 24), overridable
with `hour12`. The field parses a trailing time after a space (`06/15/2026 2:30pm`
or `… 14:30`), and typing a 24-hour hour into a 12-hour field converts it (18 → 6 PM).
...
`Calendar` splits along one seam: the `<a-calendar>` element is the **day grid**,
and the JSX wrapper adds the **navigation chrome** around it. Knowing which half
owns what tells you where to reach when you extend it, and lets a non-React consumer
rebuild the chrome from stock elements.
