...
To colour the **label and hint**, there's no prop — set the `--radio-label-color` and
`--radio-hint-color` tokens (they cascade to every option) to a `--text-N-{tone}`
value.
...
dimensions + type. To tint the label + hint, set the `--radio-label-color` /
`--radio-hint-color` tokens with the `--text-N-{tone}` values — there's no text-tone
prop. The focus ring is the global [`--focus-ring`](/colors/#focus-ring).
