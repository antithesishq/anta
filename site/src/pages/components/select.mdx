...
It's a **composed component** — an [Input](/components/input/) trigger stitched to
a [Menu](/components/menu/), not a new element. Options are a JSON array of strings
or objects (like [RadioGroup](/components/radio/) / [Tabs](/components/tabs/));
changes arrive through one **`onValueChange`**. Controlled via `value`, uncontrolled
via `defaultValue`.
...
in-menu keyboard come from `Menu`. So the same result is reachable **without React**
— the menu anchors to its previous sibling, so a field + a menu + a little glue is a
working select:
...
That glue — tracking `selected`, flipping the chevron, updating the field — is
exactly what the React `Select` packages.
