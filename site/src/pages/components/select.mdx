...
import { SelectOptionsDemo, SelectControlledDemo, SelectBasicDemo, SelectModesDemo, SelectSizeStatusDemo, SelectFilterDemo, SelectRenderOptionDemo, SelectTruncateDemo, SelectRenderIndicatorDemo, SelectTriggerDemo, SelectColumnsDemo, SelectEmptyDemo, SelectGhostDemo } from '../../components/SelectDemos'
...
**`renderTrigger(state)`** swaps the default field for your own trigger. Return one
focusable element (an Anta `Button` here); the menu anchors to it, opens it on click,
and leaves its ARIA to you, so add `aria-haspopup="menu"` and
`aria-expanded={state.open}`.

`state` carries `open`, `value`, `selected`, and `disabled`. `selected` is the
...
`status`, …) don't apply, since you own the element.
...
Whatever you return renders where the rows would. A plain string is inert; return a
`MenuItem` — like the `Create "…"` row here, which arrow-keys and Enter reach like any
option — to make it selectable.
...
for the option content (`renderOption`), the selection mark (`renderIndicator`), and
the trigger (`renderTrigger`). Still to come: **sections** (grouped options) and
**`allow_new`** (create an option from the filter query).
