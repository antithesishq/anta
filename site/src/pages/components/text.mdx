...
want emphasis. Pair with `tone` for a tint (`brand`, `success`,
`critical`, `warning`, `info`); the default `neutral` leaves it
untinted. Pass `inline` to render as `inline-block` for use in the
