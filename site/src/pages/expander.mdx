...
heading text, an inline link — copies or activates without toggling the
section. The header refuses to toggle when the click landed on an
interactive control (`<a-button>` / `ButtonCopy`, `[data-anta][role="button"]`,
a native `button` / `a[href]` / `input` / `select` / `textarea` / `label` /
editable), so a `ButtonCopy` beside the title works with no extra wiring:
...
the same contract as `Menu`'s close-on-outside-click. For a control the
detection can't recognize (a non-interactive custom widget with its own
handler), mark it `data-expander-ignore` to opt it out of the toggle:
