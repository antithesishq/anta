...
`Calendar` that opens from the trailing button. `Calendar`, the month grid it builds
on, is documented lower down.
...
Type a date and it resolves on blur or <kbd>Enter</kbd>, or open the calendar from
the trailing button and pick a day. Either way the field settles on the locale's
canonical format and the value is ISO `YYYY-MM-DD`. Focusing the field just places
the cursor to type; only the button opens the calendar.
...
sit under the grid), and owns the draft text, the lenient parse on commit, the ISO
value, and opening the menu from the trailing button. Focusing the field just types;
the button opens the menu, so picking a day and typing never fight. Nesting a stateful
component in a menu has one wrinkle: the `Calendar` (and `Tabs`) dispatch their own
