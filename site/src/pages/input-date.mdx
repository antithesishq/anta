...
`InputDate` has no host element. Although Anta exposes `buildMonth` and
`parseISODate`, building the equivalent vanilla field still means managing the
popup, localized day grid, keyboard navigation, roving focus, selection, and text
parsing yourself. Use the `InputDate` wrapper in React or Preact. For a vanilla
form, use the native `<input type="date">` control instead.
