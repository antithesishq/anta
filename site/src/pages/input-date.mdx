...
time row under the calendar: hours, minutes, an AM/PM toggle in 12-hour locales, and
a confirm (✓) button. Picking a day keeps the menu open so you can set the time. The
clock follows the locale (`en-US` is 12-hour, most others 24), overridable with
`hour12`. The field parses a trailing time after a space (`06/15/2026 2:30pm` or
`… 14:30`), and typing a 24-hour hour into a 12-hour field converts it (18 → 6 PM).

<Preview gap="16px" style="flex-direction: column; align-items: flex-start; padding-left: 40px;">
...
`tertiary` Button in its selected state), inside a form-associated `<a-calendar>`
...
selected day takes the tertiary-button selected look, a filled cell with a ring.
Today shows as a secondary Button, a resting fill.

<Preview minHeight={282}>
...
<Preview gap="24px" style="flex-wrap: wrap; align-items: flex-start;" minHeight={310}>
...
<Preview gap="24px" style="flex-wrap: wrap; align-items: flex-start;" minHeight={282}>
