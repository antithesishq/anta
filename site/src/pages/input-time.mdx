...
or just type the digits. <kbd>Home</kbd>/<kbd>End</kbd>, selection, deletion, and
...
the host or through `::part()` — the element exposes `field`, `segments`, `segment`,
`leading`, `label`, `hint`, `clear`, and `trailing` parts. Don't override the internal
