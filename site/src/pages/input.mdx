...
<Input
  ...
  readOnly
  dimActions
  trailing={<Icon shape="chevron-down" className={open ? 'chevron open' : 'chevron'} />}
/>
...
For a standard HTML form, add `data-anta` to a text-like input. Text, email,
telephone, URL, password, and number inputs keep their native editing, password
masking, keyboard, validation, and form behavior while using Anta's field chrome.
`submit` and `reset` inputs use the matching Anta button chrome; set
`priority="primary"` on a submit control when it is the form's main action.

<Preview gap="16px">
  <input data-anta style={{ marginBottom: '8px' }} type="text" name="name" placeholder="Text" />
  ...
  <input data-anta style={{ marginBottom: '8px' }} type="url" name="site" placeholder="URL" />
  <input data-anta style={{ marginBottom: '8px' }} type="password" name="password" value="password" />
  <input data-anta style={{ marginBottom: '8px' }} type="number" name="seats" value="3" min="1" />
