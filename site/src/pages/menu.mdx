...
<PropsTable component="MenuItemCopy" />
...
The surface transitions its **width** — tune the duration with
`--menu-width-duration` (`0` disables), per menu via `::part(menu)`; it's zeroed
under `prefers-reduced-motion`. A content-driven `auto` width change (a badge
appearing on a row) isn't transitionable in CSS, so only length-driven width
changes ease.

```css
a-menu.snappy::part(menu) { --menu-width-duration: 0; }   /* no width transition */
```
