...
import progressTabsDemoCode from './progress-tabs.demo.ts'
import { Basic, Statuses, ControlledSequence, Sizes, Vertical, CustomNavigation } from '../components/ProgressTabsDemo'

# ProgressTabs

A process stepper for switching between ordered phases. Each option provides a
stable `value`, a visible `label`, and a process `status`. Use [`Tabs`](/tabs/)
...
<Playground
  ...
  layout="side"
  panelHeight={470}
  initialCode={progressTabsDemoCode}
...
Add a [`TabPanel`](/tabs/#panels) with the same `value` as each option when
...
<Preview background="var(--bg-base)" align="stretch" minHeight={150}>
  <ControlledSequence client:visible={{ rootMargin: '200px' }} />
</Preview>
...
<Preview background="var(--bg-base)" align="stretch" minHeight={90}>
  <CustomNavigation client:visible={{ rootMargin: '200px' }} />
</Preview>
...
Events use the same selection contract as [`Tabs`](/tabs/#events):
...
<Disclosure title="ProgressTabs props" open>

<PropsTable component="ProgressTabs" />

</Disclosure>

<Disclosure title="Progress tab option props">

<PropsTable component="ProgressTabOption" />
