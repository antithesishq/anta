...
import { SelectFacetedBasicDemo, SelectFacetedRecencyDemo, SelectFacetedDurationDemo } from '../components/SelectFacetedDemos'
...
A number — "min duration". The editor is one `Input`, which has no popup of its own,
so it sits directly in the flyout; the value is a `number`.
...
Inline editors like `Input`, `RadioGroup`, or `Checkbox` work directly in the flyout —
they have no popup of their own. A **calendar-based** editor is the exception: `InputDate`
(and even the bare `Calendar`, which embeds a menu for its month jump) opens a menu that
can't open while nested inside the facet's menu. Edit a date **beside the trigger**
instead — the [Recency](#a-recency-filter-presets-plus-a-range) example below does exactly
this, holding `Date`s in the facet value and rendering its `InputDate` fields in the
result area. (A `custom` facet holds the value whole, so a `Date` is fine — it isn't
compared by `===` like a `single` / `multiple` option value, so there's no identity
pitfall.)

#### A recency filter: presets plus a range

A compound editor: a `RadioGroup` of presets (Today, Yesterday, Last 14 / 30 days) and
a **Custom range**. The facet value is either `{ preset }` or `{ from, to }`, and a
`resolveRange` helper turns whichever one into the concrete `{ from, to }` you filter
rows with (shown in the readout under the trigger).

The flyout holds **only the presets**. When "Custom range" is picked, the two
`InputDate` fields render **beside the trigger**, not inside the flyout — a menu-based
editor like `InputDate` opens its calendar in its own menu, which can't open while
nested inside the facet's menu. Editing the active value in the result area is the same
pattern the demo above uses for its `Select` chips.
...
// The flyout: presets only. Picking "Custom range" sets an empty { from, to }.
const recency: SelectFacet = {
  ...
  render: ({ value, onChange }) => {
    ...
    return (
      // Controlled group: apply the pick in onStateChange (onValueChange fires only
      // after `value` changes, so it can't drive a controlled RadioGroup).
      <RadioGroup
        size="small"
...
// The custom range's date fields live beside the trigger — outside the menu, so their
// calendars open. They edit value.recency directly.
const range = value.recency && 'from' in value.recency ? value.recency : null
  ...
  {range && (
    <>
      <InputDate size="small" label="From" value={range.from}
        onValueChange={(from) => setRecency({ from, to: range.to })} />
      <InputDate size="small" label="To" value={range.to} min={range.from || undefined}
        onValueChange={(to) => setRecency({ from: range.from, to })} />
    </>
  )}
</>
