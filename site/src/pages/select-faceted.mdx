...
A date, cast through a helper. Editors that open **their own popup** — the inline
`Calendar`, `InputDate`, `Select` — work in the flyout too: each opens its menu stacked
on top, and the flyout stays open while you use it. This example uses the inline
`Calendar` grid; two helpers bridge its ISO strings and the `Date` you store.
...
A `custom` facet holds the value whole, so a `Date` is fine — it isn't compared by
