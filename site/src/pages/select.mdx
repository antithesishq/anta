...
Select uses an [Anta Button](/button/) for its field and a [Menu](/menu/) for its
...
The trigger is an [Anta Button](/button/) styled as a field.
...
Select composes an [Anta Button](/button/) and a [Menu](/menu/). Reach for props first, then plain CSS or `::part` for
