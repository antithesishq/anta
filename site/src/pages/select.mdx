...
("N selected" — override the text with [`renderSummary`](#selection-summary)). A
...
```tsx folded
const [value, setValue] = useState(['eng', 'design', 'ops'])
...
built-in "one label / `N selected`". It runs while something is selected — an empty
selection still shows the `placeholder` — and takes the resolved `selected` options
(`selected.length` is the count). Return `undefined` for any case you'd rather leave
to the default, as above for the single-label case.
