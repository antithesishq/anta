...
On a `selectAll` multi-select, **`Alt`+Click** a row (⌥+Click on macOS) to *isolate*
it — clear the rest and select only that one. A cursor-following hint teaches the
accelerator; override it per row with `SelectOption.tooltip` (or `''` to suppress).
