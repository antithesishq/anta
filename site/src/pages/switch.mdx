...
<Preview gap="16px">
  <Switch tone="brand" toneScope="selected" label="Off — neutral" />
  <Switch defaultChecked tone="brand" toneScope="selected" label="On — brand" />
  <Switch defaultChecked tone="critical" toneScope="selected" label="On — critical" />
</Preview>
...
Reach for the props first: **`tone`** chooses the control color, while **`toneScope`** chooses whether it colors all states or only the checked track. `tone` accepts any CSS color; **`size`** scales the control, label, and hint. The focus ring is the global [`--focus-ring`](/colors/#focus-ring).
