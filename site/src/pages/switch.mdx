...
Switches are fully rounded by default. Pass `round` with a number (pixels) or CSS length to set the track radius; the thumb radius is 4px smaller, with a 1px minimum. Pass `round={0}` for square track corners.
...
```tsx folded
<Switch defaultChecked round={6} label="Custom corners" />
