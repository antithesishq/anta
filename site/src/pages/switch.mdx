...
Switches are fully rounded by default. Pass `round` with a number (pixels) or CSS length to set the thumb radius; the track is 2px less rounded, matching its inset around the thumb. Pass `round={0}` for square corners.

<Preview gap="16px">
  <Switch defaultChecked round={4} label="4px thumb / 2px track" />
  <Switch defaultChecked round={0} label="Square" />
</Preview>

```tsx folded
<Switch defaultChecked round={4} label="Custom corners" />
