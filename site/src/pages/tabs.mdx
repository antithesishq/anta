...
from the current location and navigate in `onStateChange` (the pick that fires *on
click* — `onValueChange` waits for `value` to change, which for a route only happens
after you've already navigated). Skip the `<TabPanel>`s, since the router renders the
view.
...
Reach for **`onStateChange`** whenever you *control* `value` — it's the one that
fires on a user pick, so it's how you learn of a change in time to apply it (a
controlled strip's `value` update, and only that, later fires `onChange` /
`onValueChange`). For an *uncontrolled* strip the element applies the pick itself,
so **`onValueChange`** is the everyday "react to the new value". **`onFocus` /
`onBlur`** report focus entering and leaving the *strip* (wired to `focusin` /
`focusout`, since focus lands on an individual tab).
